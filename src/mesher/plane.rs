...
impl PlaneTriangulation {
    ...
    fn all_vidx(&self) -> impl Iterator<Item = VIdx> {
        ...
        // TODO when applicable include constraint geometry's vertices
        // (VIdx will end up with a Constraint variant with indices)
        (0..self.vertices.len())
            .map(|x| VIdx::Real(x))
    ...
    fn add_triangle<T: Into<Triangle>>(&mut self, tri: T) -> bool {
        ...
        let edges = tri.edges();
        if edges.into_iter().any(|e| self.tris.contains_key(e)) {
            return false;
        }
        ...
        for e in edges.into_iter().cloned() {
    ...
    fn delete_triangle<T: Into<Triangle>>(&mut self, tri: T) -> bool {
        ...
        if self.tris.get(&edges[0]) == Some(&tri.2) {
            ...
            for e in to_move.edges().into_iter().cloned() {
                self.tri_idxs.insert(e, idx);
            }
            true
        } else {
