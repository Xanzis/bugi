...
pub fn lines_to_elas<'a, T: Iterator<Item = &'a str>>(mut lines: T) -> Result<ElementAssemblage, FileError> {
	let mut no = 0;
	...
	while let Some(line) = lines.next() {
		...
		state = match state {
			...
			ReadState::Header{idx} => {
				match idx {
					0 => {
						let (n, v) = name_and_num(no, line)?;
						if n != "version" { return Err(parse_error(no, "expected version")) }
						version = Some(v as u8);
						ReadState::Header{idx: 1}
					},
					1 => {
						let (n, v) = name_and_num(no, line)?;
						if n != "dim" { return Err(parse_error(no, "expected dimension")) }
						dim = Some(v);
						ReadState::Header{idx: 2}
					},
					2 => {
						let (n, m) = name_and_name(no, line)?;
						if n != "material" { return Err(parse_error(no, "expected material")) }
						material = Some(make_material(no, m)?);
						ReadState::Header{idx: 3}
					}
					_ => {
						if line == "$EndHeader" { ReadState::NodeStart }
						else {
							...
							else { return Err(parse_error(no, "unrecognized header field")) }
							ReadState::Header{idx: 3}
						}
					},
				}
			...
			ReadState::NodeStart => {
				if line != "$Nodes" { return Err(parse_error(no, "expected $Nodes")) }
				ReadState::Nodes{in_body: false}
			},
			ReadState::Nodes{in_body} => {
				match in_body {
					false => {
						let (n, v) = name_and_num(no, line)?;
						if n != "count" { return Err(parse_error(no, "expected count")) }
						node_count = v;
						ReadState::Nodes{in_body: true}
					},
					true => {
						if line == "$EndNodes" {
							if nodes.len() != node_count { return Err(parse_error(no, "did not read specified node count")) }
							ReadState::ElementStart
						} else {
							let (id, p) = id_and_point(no, line)?;
							if id != nodes.len() { return Err(parse_error(no, "out-of-order node id")) }
							nodes.push(p);
							ReadState::Nodes{in_body: true}
						}
					},
			...
			ReadState::ElementStart => {
				if line != "$Elements" { return Err(parse_error(no, "expected $Elements")) }
				ReadState::Elements{in_body: false}
			},
			ReadState::Elements{in_body} => {
				match in_body {
					false => {
						let (n, v) = name_and_num(no, line)?;
						if n != "count" { return Err(parse_error(no, "expected count")) }
					...
					true => {
						if line == "$EndElements" {
							if elements.len() != el_count { return Err(parse_error(no, "did not read specified element count")) }
							ReadState::ConstraintStart
						} else {
							let (id, tp, ns) = id_type_and_list(no, line)?;
							if id != elements.len() { return Err(parse_error(no, "out-of-order element id")) }
							elements.push((tp, ns));
							ReadState::Elements{in_body: true}
						}
					},
			...
			ReadState::ConstraintStart => {
				if line != "$Constraints" { return Err(parse_error(no, "expected $Constraints")) }
				ReadState::Constraints{in_body: false}
			},
			ReadState::Constraints{in_body} => {
				match in_body {
					false => {
						let (n, v) = name_and_num(no, line)?;
						if n != "count" { return Err(parse_error(no, "expected count")) }
					...
					true => {
						if line == "$EndConstraints" {
							if constraints.len() != cons_count { return Err(parse_error(no, "did not read specified constraint count")) }
							ReadState::ForceStart
						} else {
							let (id, tp, ds) = id_type_and_list(no, line)?;
							if id >= nodes.len() { return Err(parse_error(no, "constraint refers to out-of-bounds node index")) }
			...
			ReadState::ForceStart => {
				if line != "$Forces" { return Err(parse_error(no, "expected $Forces")) }
				ReadState::Forces{in_body: false}
			},
			ReadState::Forces{in_body} => {
				match in_body {
					false => {
						let (n, v) = name_and_num(no, line)?;
						if n != "count" { return Err(parse_error(no, "expected count")) }
					...
					true => {
						if line == "$EndForces" {
							if forces.len() != force_count { return Err(parse_error(no, "did not read specified force count")) }
							ReadState::End
						} else {
							let (id, p) = id_and_point(no, line)?;
							if id >= nodes.len() { return Err(parse_error(no, "force refers to out-of-bounds node index")) }
							forces.push((id, p));
							ReadState::Forces{in_body: true}
						}
					},
			...
			ReadState::End => {
				return Err(parse_error(no, "unexpected line at end of file"))
			}
		};
	}

	if version != Some(1) { return Err(parse_error(no, "unsupported version")) }
...
fn name_and_num<'a>(no: usize, line: &'a str) -> Result<(&'a str, usize), FileError> {
	...
	let name = l_split.next()
		.ok_or(parse_error(no, "no name on line"))?;
	let num_str = l_split.next()
...
fn id_and_point(no: usize, line: &str) -> Result<(usize, Point), FileError> {
	...
	let id = l_split.next()
		...
		.parse::<usize>().or(Err(parse_error(no, "bad uint")))?;

	let vals_str = l_split.next()
		.ok_or(parse_error(no, "expected float series"))?;
	let mut vals: Vec<f64> = Vec::new();
	for val_str in vals_str.split("/") {
		vals.push(val_str.parse::<f64>().or(Err(parse_error(no, "bad float")))?);
	}

	let p: Point = vals.try_into().or(Err(parse_error(no, "couldn't assemble Point")))?;
...
fn id_type_and_list(no: usize, line: &str) -> Result<(usize, u8, Vec<usize>), FileError> {
	...
	let tp = l_split.next()
		...
		.parse::<u8>().or(Err(parse_error(no, "bad uint")))?;

	let ns_str = l_split.next()
		.ok_or(parse_error(no, "expected id series"))?;
	let mut ns: Vec<usize> = Vec::new();
	for n_str in ns_str.split("/") {
		ns.push(n_str.parse::<usize>().or(Err(parse_error(no, "bad uint")))?);
	}
...
fn make_constraint(no: usize, tp: u8, dims: Vec<usize>) -> Result<Constraint, FileError> {
	...
	match tp {
		0 => {
			if dims.len() != 3 { Err(parse_error(no, "constraint requires three dimensional specifiers")) }
			else {
