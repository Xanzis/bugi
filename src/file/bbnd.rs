	...
	while let Some(line) = lines.next() {
        ...
        match words[0] {
        	"polygon" => {
                ...
                if words.len() != 1 { return Err(FileError::parse("unexpected word after polygon")) }
            ...
            "thickness" => {
                if words.len() != 2 { return Err(FileError::parse("expected thickness value")) }
                let val: f64 = words[1].parse()
                    .or_else(|_| Err(FileError::parse("invalid thickness value")))?;
                b.set_thickness(val);
            },
            "material" => {
                if words.len() != 2 { return Err(FileError::parse("expected material type")) }
                let mat: Material = words[1].parse()
                    .or_else(|_| Err(FileError::parse("invalid material specifier")))?;
                b.set_material(mat);
            },
            "distributed_force" => {
                if words.len() != 5 { return Err(FileError::parse("expected vertex labels, force vector"))}
            ...
            "distributed_constraint" => {
                if words.len() != 4 { return Err(FileError::parse("expected vertex labels, constraint")) }
                if let (Some(ida), Some(idb)) = 
                    (named_vertices.get(&words[1].to_string()).cloned(),
                    named_vertices.get(&words[2].to_string()).cloned()) {
