...
impl Matrix {
    ...
    pub fn mutate<F>(&mut self, loc: (usize, usize), mut f: F)
    where
        F: FnMut(&mut f64),
    {
    ...
    pub fn mutate_row<F>(&mut self, i: usize, mut f: F)
    where
        F: FnMut(&mut f64),
    {
    ...
    pub fn mutate_col<F>(&mut self, i: usize, mut f: F)
    where
        F: FnMut(&mut f64),
    {
    ...
    pub fn from_rows(rows: Vec<Vec<f64>>) -> Self {
        ...
        for r in rows.iter() {
            if r.len() != ncols {
                panic!("inconsistent row lengths in Matrix::from_rows")
            }
            res.data.extend(r);
        }
        res
    ...
    pub fn from_points_row(points: Vec<Point>) -> Self {
        ...
        for p in points.into_iter() {
            if p.dim() != dim {
                panic!("inconsistent point dimensionalities in Matrix::from_points_row");
            }
            rows.push(p.into());
        }
    ...
    pub fn add_block_below(&mut self, other: &Matrix) {
        if self.ncol != other.ncol {
            panic!("inconsistent row lengths in Matrix::add_block_below");
        }
...
impl<'a> Add for &'a Matrix {
    ...
    fn add(self, rhs: Self) -> Self::Output {
        ...
        res.data
            .extend(self.data.iter().zip(rhs.data.iter()).map(|x| x.0 + x.1));
        res
    }
}
