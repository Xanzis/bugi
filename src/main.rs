...
fn main() -> Result<(), BugiError> {
	...
	let args = Args::compile(args.collect())
		.map_err(|e| {help(); e})?;
...
fn linear(args: Args) -> Result<(), BugiError> {
	...
	match out_path.to_str() {
		...
		None => {
			return Err(BugiError::arg_error("out path is not valid unicode"))
		}
	}
	...
	fn compile(env_args: Vec<String>) -> Result<Self, BugiError> {
		...
		for mut s in env_args.into_iter() {
			if s.as_str().starts_with("--") {
				...
				s.drain(..1);
				let words: Vec<String> = s.as_str().split("=")
		...
		Ok(Args {command, args, flags, named_vals})
	}
